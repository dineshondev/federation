...
export function extractCoreFeatureImports(url: FeatureUrl, directive: Directive<SchemaDefinition, CoreOrLinkDirectiveArgs>): CoreImport[] {
  ...
  // Note: up to this point, we've kind of cheated with typing and force-casted `importArg` to a list of (string | CoreImport) (for convenience)
  // but because `link__Import` is a scalar, there is no graphQL-side validation of the values and we need to manually check that is is indeed
  // either a string or a valid `CoreImport` value.
  const { imports, errors } = validateCoreFeatureImports(url, directive, args.import);
...
// Note that the `import` arg type is a list of the scalar `link__Import`, so in practice, any graphQL value can have been provided in it,
// hence the `any`.
function validateCoreFeatureImports(
  url: FeatureUrl,
  directive: Directive<SchemaDefinition>,
  importArgValue: any[] | undefined
): { imports: CoreImport[], errors: GraphQLError[] } {
  ...
  for (const elt of importArgValue) {
    ...
    if (name) {
      imports.push(elt as CoreImport);
