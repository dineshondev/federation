...
function buildFieldDefinitionInner(
  ...
  field.type = ensureOutputType(type, field.coordinate, fieldNode, errors);
...
function ensureOutputType(type: Type, what: string, node: ASTNode, errors: GraphQLError[]): OutputType | undefined {
...
function ensureInputType(type: Type, what: string, node: ASTNode, errors: GraphQLError[]): InputType | undefined {
  if (isInputType(type)) {
    return type;
  } else {
...
function buildArgumentDefinitionInner(
  ...
  arg.type = ensureInputType(type, arg.coordinate, inputNode, errors);
...
function buildInputFieldDefinitionInner(
  ...
  const type = buildTypeReferenceFromAST(fieldNode.type, field.schema());
  field.type = ensureInputType(type, field.coordinate, fieldNode, errors);
